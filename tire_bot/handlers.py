...
async def pay_start(message: Message | CallbackQuery, state: FSMContext):
    if isinstance(message, Message):
        await message.delete()
        await message.answer('1. Выберите категорию', reply_markup=await available_kb(CategoryDB))
        await state.set_state(Pay.wait_for_category)
    else:
        await message.message.delete_reply_markup()
        await message.message.answer('1. Выберите категорию', reply_markup=await available_kb(CategoryDB))
        await state.set_state(Pay.wait_for_category)
...
async def category_chosen(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(text='2. Кто оплатил', reply_markup=await available_kb(PayerDB))
    await state.update_data(message_id=call.message.message_id)
...
async def object_chosen(message: Message, state: FSMContext):
    ...
    user_data = await state.get_data()
    await message.delete()
    await message.bot.edit_message_text(text=f'4. Напишите стоимость',
                                        chat_id=message.chat.id,
                                        message_id=user_data['message_id'])
...
async def price_chosen(message: Message, state: FSMContext):
    ...
    await message.delete()
    await message.bot.edit_message_text(text=
                                        f'Категория: {user_data["chosen_category"]}\n'
...
async def send_chosen(call: CallbackQuery, state: FSMContext):
    ...
    await to_pay_bd(call.message.from_user.id, call.message.from_user.username, user_data["chosen_category"],
                    user_data["chosen_payer"], user_data["chosen_object"], user_data["chosen_price"])
    await state.clear()
